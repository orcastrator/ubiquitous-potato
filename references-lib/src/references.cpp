...
const std::regex BibleReference::wholeChapter = std::regex("(\\d{0,1}[\\w\\s]+) (\\d+)");
const std::regex BibleReference::wholeMultiChapter = std::regex("(\\d{0,1}[\\w\\s]+) (\\d+)[-](\\d+)");
...
3 john 1 supported (only refers to chapter, not verse. Add support for verse-only for numbers > 1)
3 john 1:1 supported
3 john 1:1-2 supported
3 john 1-2 supported
3 john 1:1-1:2 supported
*/
std::pair<bible_reference, bible_reference> BibleReference::parseReference(const std::string inputReference) {
	...
	else if (std::regex_search(inputReference.begin(), inputReference.end(), matches, wholeMultiChapter)) {
		std::string book = matches[1];
		int chapterBegin = std::stoi(matches[2]);
		int chapterEnd = std::stoi(matches[3]);
		int verse = maxVersesInChapter;
		bible_reference begin = std::make_tuple(book, chapterBegin, 1);
		bible_reference end = std::make_tuple(book, chapterEnd, verse);
		return std::make_pair(begin, end);
	}
	else if (std::regex_search(inputReference.begin(), inputReference.end(), matches, wholeChapter)) {
		std::string book = matches[1];
		int chapter = std::stoi(matches[2]);
		int verse = maxVersesInChapter;
		bible_reference begin = std::make_tuple(book, chapter, 1);
		bible_reference end = std::make_tuple(book, chapter, verse);
		return std::make_pair(begin, end);
	}
...
std::string BibleReference::correction(const std::vector<std::string> referenceList, const std::string inputBook) {
	...
	std::vector<std::string> corrections = known(referenceList, oneEdit);
